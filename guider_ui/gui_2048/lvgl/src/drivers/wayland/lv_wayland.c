...
void lv_wayland_wait_flush_cb(lv_display_t * disp)
{
    ...
    if(!window || window->closed || window->shall_close) {
        return;
    }
    ...
    /* Calculate timeout based on LVGL refresh period:
      * Use 1.5x refresh period, minimum 5ms, maximum 50ms */
    int timeout_ms = (LV_DEF_REFR_PERIOD * 3) / 2;
    timeout_ms = LV_MAX(timeout_ms, 5);
    timeout_ms = LV_MIN(timeout_ms, 50);

    int poll_count = 0;
    const int max_polls = 3;

    while(initial_frame_counter == window->frame_counter && poll_count < max_polls) {
        /* Check if window state changed during wait */
        if(window->closed || window->shall_close) {
            break;
        }

        int ret = poll(&lv_wl_ctx.wayland_pfd, 1, timeout_ms);
        if(ret < 0) {
            /* Error occurred */
            LV_LOG_WARN("Poll error in wait_flush_cb: %s", strerror(errno));
            break;
        } else if(ret == 0) {
            /* Timeout occurred */
            poll_count++;
        ...
        /* Handle input and check if frame counter updated */
        handle_input();
        poll_count++;
    }

    if(poll_count >= max_polls) {
        LV_LOG_WARN("Frame sync timeout after %dms total", timeout_ms * max_polls);
    }
}
